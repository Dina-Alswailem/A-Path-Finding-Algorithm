...
class node:
    ...
    # Methods to set node properties
    def reset(self):
        self.color = CLEAN
    ...
    # Initialize node visualization on the grid
...
# Reconstruct and visualize the path found by A* algorithm
...
# Define a function to create a grid with obstacles
...
# Function to draw a grid of lines on the window surface
...
# Function to draw the grid and nodes on the window surface
def produce(win, grid, rows, width):

    # Fill the window with a clean color
    win.fill(CLEAN)

    # Initialize the visual representation of each node
    for row in grid:
        for tnode in row:
            tnode.init_map(win)
